...
void MasternodeList::StartAlias(std::string strAlias)
{
    ...
    BOOST_FOREACH (CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
        if (mne.getAlias() == strAlias) {
            ...
            bool fSbwrgocoiness = RpcStartMasternode(strAlias,strOverall,strError,strResult);
            if(fSbwrgocoiness){
...
void MasternodeList::updateMyMasternodeInfo(QString strAlias, QString strAddr, CMasternode* pmn)
{
    ...
    if (pmn) {
        ...
        switch (pmn->Level())
        {
            ...
            case 2: mnLevelText = "Medium"; if (roi2 > 1) tLuck = ((masternodeCoins/COIN) / roi2)*100; break;
            case 3: mnLevelText = "Full"; if (roi3 > 1) tLuck = ((masternodeCoins/COIN) / roi3)*100; break;
        }
    }
