...
if [ "$DEPLOY_TEST_BUILDS" = "true" ] && [ "$BUILD_ONLY_DEPENDS" = "false" ]; then
  ...
  if [ "$ADDWINSETUP" = "true" ]; then DOCKER_EXEC cd "$TRAVIS_BUILD_DIR/release" && pwd && cp -a *.exe "$TRAVIS_BUILD_DIR/uccrelease/" && cd "$TRAVIS_BUILD_DIR/uccrelease" && strip *.exe && ls -Rha && zip -r UCC-$REASON-$TRAVIS_BRANCH.zip * && ls; fi
